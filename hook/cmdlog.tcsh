...
#
# tcsh has no BASH_SOURCE equivalent. Set __cmdlog_dir before sourcing
# to override the install location:
#   set __cmdlog_dir = /custom/path; source $__cmdlog_dir/hook/cmdlog.tcsh
...
# Use __cmdlog_dir if set by the user, otherwise default to ~/.cmdlog
if (! $?__cmdlog_dir) then
    set __cmdlog_dir = ~/.cmdlog
endif
...
# Recording alias.
alias __cmdlog_do_record 'set __cmdlog_ec = $status ; set __cmdlog_h = "`history -h 1`" ; if ("$__cmdlog_h" != "") $__cmdlog_dir/cmdlog record tcsh $cwd $__cmdlog_ec "$__cmdlog_h"'
...
alias cmdlog 'set __cmdlog_a1 = \!:1 ; set __cmdlog_sel = "" ; set __cmdlog_eval = 0 ; set __cmdlog_method = "" ; set __cmdlog_tmpf = /tmp/.cmdlog_hist.$$ ; if ("$__cmdlog_a1" == "list") set __cmdlog_method = `$__cmdlog_dir/cmdlog config inject.tcsh` ; if ("$__cmdlog_a1" == "list") set __cmdlog_sel = `$__cmdlog_dir/cmdlog \!*` ; if ("$__cmdlog_sel" != "") set __cmdlog_eval = 1 ; if ($__cmdlog_eval) if ("$__cmdlog_method" != "tiocsti") echo "$__cmdlog_sel" > "$__cmdlog_tmpf" ; if ($__cmdlog_eval) if ("$__cmdlog_method" != "tiocsti") history -L "$__cmdlog_tmpf" ; if ($__cmdlog_eval) rm -f "$__cmdlog_tmpf" ; if ($__cmdlog_eval) $__cmdlog_dir/cmdlog inject tcsh "$__cmdlog_sel" ; if ("$__cmdlog_a1" != "list") $__cmdlog_dir/cmdlog \!*'
